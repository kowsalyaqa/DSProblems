...
public class FindMaxAverageSlidingWindowApproach {
	 ...
	 *  int[] nums={1,5,2,3,7,1};
		...
		4.Loop using nums.length to add sum for remains elements in the array
			...
			e)Increment Pointer by 1,Subtract the the first element from sum 
	...
	public  double highestSum(int[] nums,int k){
		if(nums.length<k) return 0;
		int sum=0,pointer=0;
		float average=0,max=Integer.MIN_VALUE;;
		while(pointer<k-1) {
			sum +=nums[pointer++];
		}
		...
		while(pointer<nums.length) {
			...
			average=Float.valueOf(sum)/Float.valueOf(k);
		...
		System.out.println(Double.valueOf(max));
		return max;
	}
	@Test
	public  void positive() {
		...
		double actual=highestSum(nums,k);
